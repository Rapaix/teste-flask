...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort teste_flask/
	$(ENV_PREFIX)black -l 79 teste_flask/
	$(ENV_PREFIX)black -l 79 tests/
	$(ENV_PREFIX)black -l 79 .github/templates/

.PHONY: lint
lint:             ## Run pep8, black, mypy linters.
	$(ENV_PREFIX)flake8 teste_flask/
	$(ENV_PREFIX)flake8 .github/templates/
	$(ENV_PREFIX)black -l 79 --check teste_flask/
	$(ENV_PREFIX)black -l 79 --check tests/
	$(ENV_PREFIX)black -l 79 --check .github/templates/
	$(ENV_PREFIX)mypy --ignore-missing-imports teste_flask/

.PHONY: test
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=teste_flask -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > teste_flask/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add teste_flask/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@rm -rf .venv
	@poetry init --name=teste_flask --author=Rapaix
